...
/*
2012-12-22:
doing v2.00alpha to keep up with the dataset components 
* NOTE!!! this form is no longer set to be self-contained - I have removed fields like
	mode
	dataset
	datasetTable
	and changed the submission method (now it buffers g.submode, submits and then resets)
...
if($useStatusFilterOptions){
	...
	if(!count($inStatusSet)){
		...
		if($a=q("SELECT $statusFilterIDField FROM $statusFilterTable WHERE $statusFilterQueryWhere ORDER BY $statusFilterIDField DESC LIMIT $statusFilterDefaultShown", O_COL)){
			foreach($a as $v){
				q("REPLACE INTO bais_settings SET UserName='".($filterGadgetUserName ? $filterGadgetUserName : ($_SESSION['systemUserName'] ? $_SESSION['systemUserName'] : $GLOBALS['PHP_AUTH_USER']))."', vargroup='$dataset', varnode='filter".$dataset."Status', varkey='$v', varvalue=1");
