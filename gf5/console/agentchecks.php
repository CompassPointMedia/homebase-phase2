...
if(!$Agents_username){
	mail($developerEmail, 'Error file '.__FILE__.', line '.__LINE__,get_globals($err='Agent username not passed'),$fromHdrBugs);
	header('Location: agentchecks_pre.php');
	exit($err);
}
...
$navObject='Checks_ID';
$updateMode='updateCheck';
$insertMode='insertCheck';
$deleteMode='deleteCheck';
...
$ids=q("SELECT h.ID FROM finan_headers h, finan_transactions t, gl_LeasesCommissions lc, gl_leases l
WHERE h.HeaderType='Check' AND h.ID=t.Headers_ID AND t.ID=lc.Transactions_ID AND lc.Leases_ID=l.ID AND
(l.Agents_UserName='$Agents_username' OR l.SubAgents_UserName='$Agents_username')
GROUP BY h.ID ORDER BY h.HeaderDate",O_COL);
...
if(strlen($$object) || $Payments_ID=q("SELECT ID FROM finan_headers, finan_payments WHERE ID=Headers_ID AND ID='".$$object."'", O_VALUE)){
	//get the record for the object
	if($a=q("SELECT * FROM finan_headers, finan_payments WHERE ID=Headers_ID AND ID='".$$object."'",O_ROW)){
		$mode=$updateMode;
		@extract($a);
...
<title><?php echo $PageTitle='Agent Commission Checks';?></title>
...
<div id="mainBody">
<div class="suite1">
<?php require('components/comp_75_agentchecks_v102.php');?>
</div>
