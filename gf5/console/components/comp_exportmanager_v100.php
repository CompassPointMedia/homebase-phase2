...
function showBatches($batches,$options=array()){
	...
	*/
	...
	foreach($batches as $Batches_ID){
		...
		if($qr['affected_rows']){
			mail($developerEmail, 'Notice in '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals($err='batches_entries pared down'),$fromHdrBugs);
		}
...
for($__i__=1; $__i__<=1; $__i__++){ //------------ break loop ----------------
	if($submode=='batchPrepare'){
		...
		if(!$haveExport){
			mail($developerEmail, 'Error in '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals($err='No records were found to export; either a batch has just been created for these records or there was a system error; an administrator has been notified'),$fromHdrBugs);
			error_alert($err);
		}
		...
		function memory_usage($msg){
			...
			if($z2>$memThreshold && !$notified2){
				$notified2=true;
				mail($developerEmail, 'Error in '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals($warn='The zipfile system has approached '.($memThreshold*100).'% of maximum usable memory ('.$iniMem.').  If the zipfile is not successfully created, please contact an administrator.'),$fromHdrBugs);
			}
		...
		if($maxFileSizeToZip=q("SELECT varvalue FROM bais_settings WHERE username='system' AND vargroup='items' AND varnode='settings' AND varkey='maxFileSizeToZip'", O_VALUE)){
			...
			$maxFileSizeToZip=4; //in megabytes
		}
		...
		ob_end_clean();
		if($err){
			mail($developerEmail, 'Error in '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals($err='error on batch subtype'),$fromHdrBugs);
		}
		q("DELETE FROM gen_batches_entries WHERE Batches_ID=$Batches_ID");
		...
		$tables=q("SHOW TABLES", O_ARRAY);
		foreach($tables as $n=>$v){
			$tables[current($v)]=current($v);
			unset($tables[$n]);
		}
		...
		*/
		...
		//mail me the view 
		mail($developerEmail, 'backup view build from '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals($notice='structure of view '.$view." just prior to update:\n\n".$str."\n\n"),'From: backups@'.$_SERVER['HTTP_HOST']);
		error_alert('View '.$view.' successfully updated');
		goto bypass;
	}
	//------ presentation output -------
...
<form id="formExportManager" name="formExportManager" action="/gf5/console/resources/bais_01_exe.php" method="post" target="w2"><?php
	if($disposition=='preExport' || $disposition=='readyExport'){
		...
		*/
		...
		<?php
		$batches=explode(',',$batches);
		showBatches($batches,array('disposition'=>$disposition));
		?>
		<p><?php
		if($disposition=='preExport'){
		...
		<?php
		$batches=q("SELECT ID FROM gen_batches WHERE Type='Export' AND SubType IN('".implode("','",array_keys($exportTypes))."') ORDER BY StartTime Desc", O_COL);
		showBatches($batches,array('disposition'=>$disposition));
		?>
		</div><?php
	}else{
