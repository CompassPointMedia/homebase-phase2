...
<div id="mainWrap">
	...
	foreach($a as $n=>$v){
		...
		$viewname=$a[strtolower($v['Tables_in_cpm180_hmr'])]=$v['Tables_in_cpm180_hmr'];
		prn($viewname);
		$view=q("SHOW CREATE VIEW `".$viewname.'`', O_ROW);
		$view=$view['Create View'];
		$view=preg_replace('/ALGORITHM.+DEFINER /i','OR REPLACE ',$view);
		if(preg_match('/,_latin1\'\' AS `My_[^`]+`/',$view)){
			prn('this has it');
			$before=$view;
			$view=preg_replace('/,_latin1\'\' AS `My_[^`]+`/i','',$view);
			if(strlen($before)>strlen($view))prn('removing '.(strlen($before)-strlen($view)).' bytes');
			prn($view);
			q($view);
			prn($qr);
		}
