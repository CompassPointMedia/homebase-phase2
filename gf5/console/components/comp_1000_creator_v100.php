...
if($modePassed){
	...
	if($modePassed==$insertMode){
		//merge with session
		if($a=$_SESSION['special']['datasets'][$Location]){
			...
			error_alert('Unable to find new dataset component in session');
		}
	}else if($modePassed==$updateMode){
		//overwrite gotten variables
		extract(stripslashes_deep($_POST));
	}
}
} //--------------------- end break loop ------------------------

//pull dataset information - whether existing or in session (new)
$insertMode='insertDatasetComponent';
$updateMode='updateDatasetComponent';
$deleteMode='deleteDatasetComponent';
if(preg_match('/^[a-f0-9]{32}$/i',$Location) && $a=$_SESSION['special']['datasets'][$Location]){
	//quasi dc present	
	extract($a);
	$mode=$insertMode;
	...
	$_SESSION['special']['datasets'][$Location]=array(
		'starttime'=>time(),
	);
	$mode=$insertMode;
}

//write form post over session or global variables
extract($_POST);
...
$refresh=preg_replace('/^&*Location=[^&]*/','',$_SERVER['QUERY_STRING']);
...
<div id="datasetCreator">
	...
	<br />
	Creator: 
	<input name="datasetCreator" type="text" id="datasetCreator" value="<?php echo $datasetCreator ? h($datasetCreator) : $_SESSION['firstName'] . ' ' . $_SESSION['lastName']?>" />
	<br />
	Contact email: 
	...
	if($a=$availableCols[$datasetGroup][$modApType][$modApHandle]['scheme']){
		...
		<div id="col_<?php echo $i?>" class="datasetCol">
			<div class="datasetColTools">
				...
				&nbsp;&nbsp;
				<a href="#" onclick="return manageCols(this,<?php echo $i?>,'deleteColumn');" title="delete this column"><div style="float:left;"><img src="/images/i/antset.delete.png" width="28" height="28" /></div></a>
			</div>
			Column name: 
	...
	*/
	...
	<?php	
	if(($mode==$insertMode && ($datasetTable || $datasetQuery)) || $mode==$updateMode){
		unset($ds);
