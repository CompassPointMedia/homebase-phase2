...
function systementry_focus($record){
	global $object,$datasetFocusPage,$datasetFocusViewDims,$recordPKField;
	if(!$datasetFocusPage)$datasetFocusPage='systementry.php';
	if(!$datasetFocusViewDims)$datasetFocusViewDims='750,700';
	$href=$datasetFocusPage.'?'.'_Profiles_ID_='.$GLOBALS['_Profiles_ID_'].'&'.$object.'_'.implode(':',$recordPKField).'='.datasetID();
...
require(current(explode('/components/',__FILE__)).'/systementry.assets.php');
	...
	*/
	if($mode=='deleteObjects'){
		...
		foreach($Objects_ID as $n=>$v){
			...
			q("DELETE FROM $object WHERE ".implode(' AND ',$str)." LIMIT 1");
			prn($qr);
		}
