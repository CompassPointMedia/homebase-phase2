...
still not getting clean batches_entries records (0 for key and errors but for importtable_amazon I I got the data in clean, i.e. extra columns)
	2012-06-28
		on import of seasons, use cf's, Creator was NULL
		on that same import, as a new table, putting quarry:Quarry_ID in Column mappings [as if I was going to rename] resulted in field not being imported
		we are proliferating ImportProfiles! fix this
		when switching between Type of data to import we need to unload the profile; when selecting a profile we need to load the Type of data to import radio button
		on import of sarah's spreadsheet:
			Additional Import Values - createdate worked for inserts but not for updates (and it was null/blank)
			results say 10 inserted but should have been 8 inserted 2 updated
...
-------------------
2012-06-28:
	import of new table worked great exc. that we didn't have an ID field created and could have used one, OR needed to map a column which then would have []autoinc [x]primary key (and would failback if not all unique [ints])
2012-06-05:
...
DONE	import data into an existing table
DONE	* map fields
DONE	* add fields as needed
DONE(profile) * create a schema table with all these new mappings
		...
		case 'manufacturer':
			...
			if(is_numeric($v)){
				...
				if($ID=q("SELECT ID FROM finan_manufacturers WHERE Name='".addslashes($v)."'", O_VALUE)){
					...
					return q("INSERT INTO finan_manufacturers SET Name='".addslashes($v)."', CreateDate=NOW(), Creator='".(sun() ? sun() : ($_SESSION['systemUserName'] ? $_SESSION['systemUserName'] : ($GLOBALS['PHP_AUTH_USER'] ? $GLOBALS['PHP_AUTH_USER'] : 'import')))."'", O_INSERTID);
				}
			}
		break;
		case 'purchaseprice':
...
$UserName=(sun() ? sun() : ($PHP_AUTH_USER ? $PHP_AUTH_USER : 'system'));
if($ImportProfiles_ID && $a=q("SELECT VarValue FROM bais_settings WHERE UserName='$UserName' AND VarGroup='Import' AND VarNode='Custom' AND VarKey='$ImportProfiles_ID'", O_VALUE)){
...
if($submode=='documentation'){
	...
	if(!$externalBatch)$externalBatch='0';
	if(!$doNotOverwrite)$doNotOverwrite='0';
	if(!$createColumnsDynamically)$createColumnsDynamically='0';
	if($operation=='updateTable'){
		if(!$tableName)error_alert('You are choosing to import data into an existing table; select a table from the list');
		ob_start();
		q("EXPLAIN $tableName",ERR_ECHO);
		$err=ob_get_contents();
		ob_end_clean();
		if($err)error_alert('Table '.$tableName.' does not exist.  It may have been deleted, or there may have been another error');
	}
	if($operation=='updateTable' && $updateRows && (!$arrowMergeField || !$targetMergeField ))error_alert('You are choosing to UPDATE records with matching criteria.  You must specify the column in the import source, and the column in the target table, that you wish to compare');
	...
	if($Data=='(Custom import)'){
		...
		while($r=fgetcsv($fp,100000)){
			...
			if($_i_900==0){
				foreach($r as $n=>$field){
					...
					//2012-08-16; perhaps i the future we could do column1, column2 - if there was actual data in the rows below
		...
		while($r=fgetcsv($fp,100000)){
			$_i_900++;
			if(!$_i_900)continue;
			if($operation=='updateTable' && $updateRows && strlen($r[$fieldMap[strtolower($arrowMergeField)]]) && q("SELECT COUNT(*) FROM `$table` WHERE ".$targetMergeField."='".addslashes($r[$fieldMap[strtolower($arrowMergeField)]])."'", O_VALUE)){
				//update
				$action=='update';
				...
				foreach($r as $n=>$v){
					...
					}else if(strtolower(substr($v,0,5))=='php::'){
						$v=end(explode('::',$v));
						if(!preg_match('/pw|password/i',$v))$v=$GLOBALS[end(explode('::',$v))];
			...
			*/
			if($externalBatch){
				if(!$Batches_ID)$Batches_ID=q("INSERT INTO gen_batches SET StartTime=NOW(), Status='Running', Source='".addslashes($_FILES['uploadFile_1']['name'])."', Type='import', SubType='custom', Process='".str_replace('.php','',end(explode('/',__FILE__)))."', Description='$batchDescription', Notes='mode=uploadFile line ".__LINE__."', CreateDate=NOW(), Creator='$MASTER_DATABASE'", O_INSERTID);
			}
			ob_start();
		...
		if($importProfile){
			...
			if($importProfile=='insertProfile'){
				$ImportProfiles_ID=q("INSERT INTO bais_settings SET 
				UserName='$UserName',
				VarGroup='Import',
				VarNode='Custom',
				VarKey='".(q("SELECT MAX(VarKey) FROM bais_settings WHERE UserName='$UserName' AND VarGroup='Import' AND VarNode='Custom'",O_VALUE)+1)."',
				VarValue='$VarValue'", O_INSERTID);
			}else{
				//updateProfile
				q("UPDATE bais_settings SET VarValue='$VarValue' WHERE UserName='$UserName' AND VarGroup='Import' AND VarNode='Custom' AND VarKey='$Importprofiles_ID'");
			}
		}
	}else{
	...
	?><script language="javascript" type="text/javascript">
	window.parent.location='/gf5/console/importmanager.php?mode=finished&key=<?php echo $key?>';
	</script><?php
	$assumeErrorState=false;
	exit;
}
...
if($mode=='finished'){
	...
	<h1>Import Manger</h1>
	<?php 
	if($profiles=q("SELECT VarKey, VarValue FROM bais_settings WHERE UserName='$UserName' AND VarGroup='Import' AND VarNode='Custom'", O_COL_ASSOC)){
	...
	<?php 
	if($lastFile){
		?><span class="gray"><?php echo 'Last file uploaded was <strong>'.$lastFile['name'].'</strong>, '.round($lastFile['size']/1024,2).'K<br />';?></span><?php
	}
	?>
...
; compare  this to this field in the table: 
	 ...
	 <br />
	 <label><input name="doNotOverwrite" type="checkbox" id="doNotOverwrite" value="1" <?php echo !isset($doNotOverwrite) || $doNotOverwrite?'checked':''?> onchange="dChge(this);" /> When updating, do not overwrite non-blank values with blank values</label>
	 <br />
	...
	<br />
	<label>
	<input name="externalBatch" type="checkbox" id="externalBatch" value="1" onchange="dChge(this);" <?php echo !isset($externalBatch) || $externalBatch?'selected':''?> />
Create an external batch reference;</label> description of batch:
<input name="batchDescription" type="text" id="batchDescription" value="<?php echo h($batchDescription);?>" size="35" maxlength="255" onchange="dChge(this);" />
	...
	<select name="importProfile" id="importProfile" onchange="dChge(this);interlock091(this);">
      <option value="">(none)</option>
      <option value="insertProfile">Store this import as a profile</option>
