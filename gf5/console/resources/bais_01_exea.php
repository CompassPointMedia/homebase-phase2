...
switch(true){
	...
	case $mode=='deleteItem':
		...
		if(!$autosave){
			$ResourceType=1;
			if($mode==$insertMode){
				foreach(q("SELECT Grouping, COUNT(*) FROM finan_items WHERE Grouping IS NOT NULL AND ResourceType IS NOT NULL ORDER BY Grouping", O_COL_ASSOC) as $MaxGroup=>$Count){
					//ok
				}
	...
	case $mode=='exportManager': /*2012-07-04*/
		require('../components/comp_exportmanager_v100.php');
	break;
	case $mode=='deleteMapPending':
