...
if(
	...
	!q("SELECT ID FROM gf_fosterhomes WHERE ID='$ID' AND ResourceType IS NULL AND Creator='".$_SESSION['admin']['userName']."'", O_VALUE
)){
...
if($mode==$componentMode){
	...
	if($submode=='insertObject'){
		...
		Description='".$GLOBALS[$gfObjectPre.'Description']."',
		EditDate=NOW(),
		Editor='".$_SESSION['admin']['userName']."' WHERE ID='".$GLOBALS[$gfObjectPre.'_ID']."'");
		prn($qr);
	}
}
