...
<tr>
	...
	<th>Name</th>
	<th>Description</th>
	<th nowrap="nowrap">Times Used</th>
</tr>
</thead>
<tbody>
<?php
if($a=q("SELECT t.*, COUNT(DISTINCT i.ID) AS Used FROM finan_items_themes t LEFT JOIN finan_items i ON t.Name=i.Theme WHERE 1 GROUP BY t.Name ORDER BY Name", O_ARRAY)){
	$i=0;
	foreach($a as $n=>$v){
		extract($v);
		$i++;
		$j++;
		?><tr id="r_<?php echo md5($Name);?>" class="<?php echo !fmod($j,2)?'alt':''?>">
			<td><?php
			if($Used==0){
				?>[<a href="resources/bais_01_exe.php?mode=deleteTheme&Name=<?php echo urlencode($Name);?>" title="Delete this theme from the list" target="w2">delete</a>]<?php
			}else{
				?>&nbsp;<?php
			}
			?></td>
			<td>[<a href="themes.php?Name=<?php echo urlencode($Name);?>" title="Edit this theme">edit</a>]</td>
			<td><?php echo $Name;?></td>
			<td><?php echo $Description;?></td>
			<td><?php echo $Used ? $Used : 'none';?></td>
		</tr><?php
	}
	?><tr>
	<td colspan="100%"><a href="themes.php">Add new theme</a></td>
	</tr>
<?php
}
?>
