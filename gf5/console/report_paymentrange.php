...
if(!$targetDate)$targetDate='HeaderDate';
if($ReportDateFrom){
	//allow any date
...
<title><?php echo ($PageTitle='Payment Range Report - '.$AcctCompanyName);?></title>
...
Currently signed in: <strong><?php echo $_SESSION['admin']['firstName'] . ' '. $_SESSION['admin']['lastName']?></strong>
	...
	<div id="toolbar1" class="printhide">
		Show Payments 
		  <select name="targetAttribute" id="targetAttribute">
		    <option value="HeaderDate" <?php echo $targetDate=='HeaderDate'?'selected':''?>>dated</option>
		    <option value="CreateDate" <?php echo $targetDate=='CreateDate'?'selected':''?>>entered</option>
	      </select>
	    between: <img align="absbottom" onClick="show_cal(this,this.nextSibling.id);" title="click this to select a date" alt="select date" class="calIcon" src="/images/i/calendar1.png"/>
		<input name="ReportDateFrom" type="text" id="ReportDateFrom" value="<?php echo date('m/d/Y',strtotime($ReportDateFrom));?>" size="14" />
		and
		<img align="absbottom" onClick="show_cal(this,this.nextSibling.id);" title="click this to select a date" alt="select date" class="calIcon" src="/images/i/calendar1.png"/>
		<input name="ReportDateTo" type="text" id="textfield4" value="<?php echo date('m/d/Y',strtotime($ReportDateTo));?>" size="14" />
...
<div class="suite1">
<?php 
$showHeaderBreaks=true;
require('components/comp_103_report_payment_range.php');
?>
<table class="yat2">
  <tr>
    <td><h2>Payment Total:</h2></td>
    <td class="tr"><h2>$<?php echo number_format($runningBalance,2);?></h2></td>
  </tr>
  <tr>
    <td><h2>Nbr. Payments:</h2></td>
    <td class="tr"><h2><?php echo $paymentCount;?></h2></td>
  </tr>
</table>
</div>
 ��</div>
