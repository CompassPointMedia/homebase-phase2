...
2012-12-29: this piece of crap is screwed up.  I FEEL LIKE GIVING UP - but I will not, ever at any time.
	gen_batches is not filling in Profiles_ID
	gen_batches_entries has never worked
...
function sql_attributes($field,$attributes,$options=array()){
	global $sql_attributes;
	extract($options);
	if(!isset($notNull))$notNull=false;
	extract($attributes);
	switch(true){
		case $integrity==DATA_BOOL:
			return 'TINYINT(1) UNSIGNED'.($notNull && !$has_null ? ' NOT NULL':'').' DEFAULT 0';
		case $integrity==DATA_INT:
			...
			$str=($length>9?'BIG':'').'INT('.$length.')'.($notNull && !$has_null ? ' NOT NULL':'');
			...
			if(!$sql_attributes['primary_key_declared'] && $mode=='insert' && $unique && $replete){
				$sql_attributes['primary_key_declared']=true;
				$str.=' AUTO_INCREMENT PRIMARY KEY';
			}
			return $str;
		case $integrity==DATA_FLOAT:
			return 'FLOAT('.($length+1).','.($decimals?$decimals:'2').')'.($notNull && !$has_null ? ' NOT NULL':'');
		case $integrity==DATA_DATE:
			return 'DATE'.($notNull && !$has_null ? ' NOT NULL':'');
		case $integrity==DATA_DATETIME:
			return 'DATETIME'.($notNull && !$has_null ? ' NOT NULL':'');
		case !$integrity:
		case $integrity==DATA_STRING:
			return 'CHAR('.($length ? $length+2 : 20).')'.($notNull && !$has_null ? ' NOT NULL':'').' DEFAULT \'\'';
		case $integrity==DATA_TEXT:
			return 'TEXT'.($notNull && !$has_null ? 'NOT NULL':'');
		case $integrity==DATA_LONGTEXT:
			return 'LONGTEXT'.($notNull && !$has_null ? ' NOT NULL':'');
		default:
		...
		case 'manufacturer':
			...
			if(is_numeric($v)){
				...
				if($ID=q("SELECT ID FROM finan_manufacturers WHERE Name='".addslashes($v)."'", O_VALUE)){
					...
					return q("INSERT INTO finan_manufacturers SET Name='".addslashes($v)."', CreateDate=NOW(), Creator='".($_SESSION['admin']['userName'] ? $_SESSION['admin']['userName'] : ($_SESSION['systemUserName'] ? $_SESSION['systemUserName'] : ($GLOBALS['PHP_AUTH_USER'] ? $GLOBALS['PHP_AUTH_USER'] : 'import')))."'", O_INSERTID);
				}
			}
		break;
		case 'purchaseprice':
...
if($submode=='documentation'){
	...
	if($Data=='(Custom import)'){
		...
		if($operation=='updateTable'){
			...
			if($createColumnsDynamically){
				foreach($fields as $n=>$v){
					...
					$sql.='ADD `'.$v.'`';
					$sql.=' '.sql_attributes($v,$data[$fieldMap[$n]]);
			...
			foreach($fields as $n=>$v){
				...
				$sql.='`'.$v.'`';
				$sql.=' '.sql_attributes($v,$data[$fieldMap[$n]],array('mode'=>'insert'));
				$sql.=','."\n";
			}
			if($ctrlFields)
			foreach($ctrlFields as $n=>$v){
			...
			mail($developerEmail, 'Error in '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals('create table query - review'),$fromHdrBugs);
		}
		if($sql){
			ob_start();
			q($sql, ERR_ECHO);
		...
		if($importProfile){
			...
			if($importProfile=='insertProfile'){
				...
				VarGroup='Import',
				VarNode='Custom',
				VarKey='".((int)q("SELECT MAX(CAST(VarKey AS UNSIGNED)) FROM bais_settings WHERE UserName='".sun()."' AND VarGroup='Import' AND VarNode='Custom'",O_VALUE)+1)."',
	...
	?>
	window.parent.location='<?php echo $file;?>?mode=finished&key=<?php echo $s_key?>';
	</script><?php
	eOK();
...
<input name="batchDescription" type="text" id="batchDescription" value="<?php echo h($batchDescription);?>" size="35" maxlength="255" onchange="dChge(this);" />
  ...
  &nbsp;
	...
	if(minroles()<=ROLE_ADMIN)$adminMode=1;
