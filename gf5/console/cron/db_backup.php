...
<title><?php echo $AcctCompanyName?> FCM ver2.0 - Database Backup</title>
...
Database to backup: 
  <input name="db" type="text" id="db" value="<?php echo $db?>" />
  back up to: 
  <input name="bk" type="text" id="bk" value="*_bk<?php echo date('z')+1;?>" />
  <br />
  <input type="submit" name="Submit" value="Backup" />
  <?php 
  if($db && $bk){
  	?><input type="button" name="button" value="Start Over" onclick="window.location='db_backup.php';" /><?php
  }
  ?>
</form>
<?php
if($_POST['db'] && $_POST['bk']){
	$cnx=array('localhost','relatebase','secret','cpm180');
	$bk=str_replace('*',$db,$bk);
	q("DROP DATABASE $bk", ERR_ECHO, $cnx);
	q("CREATE DATABASE $bk", $cnx);
	$a=q("SHOW TABLES IN $db", O_ARRAY, $cnx);
	foreach($a as $n=>$v){
		$tables[strtolower($v['Tables_in_'.$db])]=$v['Tables_in_'.$db];
	}
	ksort($tables);
	foreach($tables as $n=>$v){
		echo 'creating table or view '.$v.'.. ';
		q("CREATE TABLE $bk.$v SELECT * FROM $db.$v", $cnx);
		echo 'success<br />';
	}
