...
/*
2012-11-05:
this is being developed in blue skies aviation also - so be careful about changes 

THINGS THAT NEED DONE
2012-11-05:
* save and close and prob. save and new not working
* populate the field with the database table default value if none other set (for example Version=1.0 in system_profiles)
* pull over the new bottom tabs from my standard coding lib on relatebase
* back-update these entries in function sql_table_relationships():
	global $refreshRelations
	...
	foreach($fields as $v){
		...
		*/
		...
		switch($type){
			...
			break;
			case 'checkbox':
				?><input type="hidden" name="<?php echo $name;?>" value="0" /><input type="checkbox" name="<?php echo $name;?>" <?php foreach($output as $o=>$w)echo $o.'="'.$w.'" ';?><?php if(!$output['value'])echo ' value="1"';?> /><?php
			break;
			case 'radio':
