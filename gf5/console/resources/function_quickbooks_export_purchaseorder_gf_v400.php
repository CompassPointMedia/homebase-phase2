...
function quickbooks_export_purchaseorder($where='', $options=array()){
	global $quickbooks_export_purchaseorder, $qr, $fl, $ln, $fromHdrBugs, $developerEmail;
	if(!$quickbooks_export_purchaseorder['test']){
	$quickbooks_export_purchaseorder['test']=true;
	error_alert('test',true);
	}
	/***************************
	...
	options:
		...
		print [true] - set the  as ToBePrinted on the EXPORT (not the database record)
		structureOverrides[QBKSFIELDNAME]=SQLEXPRESSION..
	TODO:
	account is hard-coded as accounts rec. - make what the real system name is
	TRNSTYPE = "PURCHORD" - same as above
	purchaseorder number - special "w" @front
	...
	****************************/
	...
	$quickbooks_export_purchaseorder=array();

	//current header I have
	$quickbooks_export_purchaseorder['header']= <<<aleftarightarockstep
...
aleftarightarockstep;
	...
	//declaration line for a purchaseorder
	...
	$structure=array(
		'REFNUM'=>'a.ID',
		'NAME'=>"b.VendorName",
		'ACCNT'=>"'Purchase Orders'",
		'TEMPLATE'=>"'DEFAULT'",
		'DATE'=>"CONCAT(IF(DATE_FORMAT(a.InvoiceDate,'%c')<10,'0',''),DATE_FORMAT(a.InvoiceDate,'%c'),'/',IF(DATE_FORMAT(a.InvoiceDate,'%e')<10,'0',''),DATE_FORMAT(a.InvoiceDate,'%e/%y'))",
		'DOCNUM'=>"IF(a.InvoiceNumber, a.InvoiceNumber, a.ID)",
		...
		'INVMEMO'=>"'Thank You'",
		'TOPRINT'=>($print ? "'Y'" : "'N'"),
		'MEMO'=>"'Please review this purchase order for accuracy - thank you'",
		'TRNSTYPE'=>'"PURCHORD"',
		'TIMESTAMP'=>time(),
		'SPL_TRNSTYPE'=>'"PURCHORD"',
		'SPL_TIMESTAMP'=>time()
	);
	...
	LEFT JOIN gf_fosterhomes b ON a.Vendors_ID = b.ID  /* no heirarchy present */
	...
	if($setAsExported){
		q("UPDATE finan_invoices a SET a.ToBeExported=0, a.ExportTime=".($setExportTime ? "'".$setExportTime."'" : 'NOW()').", a.Exporter='".(sun() ? sun() : 'system')."' WHERE $where $filterWhere");
	}
...
aleftarightarockstep;
		...
		foreach($result as $rd){
			if(is_null($rd['NAME'])){
				...
				$quickbooks_export_purchaseorder['purchaseorders'][]=$rd['REFNUM'];
				...
				if($i==1 || $rd['DOCNUM']!==$buffer){
					//this is the beginning of a new purchaseorder
					$buffer=$rd['DOCNUM'];
					$r++;
					...
					//print the purchaseorder wrapper -------------------------------------------------------
					$records.="TRNS";
					foreach($trns as $v){
		...
		foreach($invAmtKey as $n=>$v){
			$records=str_replace($v,-abs($invAmt[$n]),$records);
		}
		
		//globalize used items and accounts
		$quickbooks_export_purchaseorder['records']=$c.$records;
		$quickbooks_export_purchaseorder['customers']=$customers;
		$quickbooks_export_purchaseorder['accounts']=$accounts;
		$quickbooks_export_purchaseorder['items']=$items;
		return true;
	}
}
?>