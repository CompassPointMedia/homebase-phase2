...
if(!function_exists('parse_javascript_gmt_date')) require($_SERVER['DOCUMENT_ROOT'] . '/functions/function_parse_javascript_gmt_date_v120.php');
...
if(($m=$_COOKIE['m']) && ($Machines_ID=q("SELECT ID FROM system_machines WHERE UniqueKey='$m'", O_VALUE))){
	...
	if($m=$_COOKIE['m']){
		...
		mail($developerEmail, 'cookies.m is present but no longer matches; notice file '.__FILE__.', line '.__LINE__,get_globals(),$fromHdrBugs);
	}
	if(!$_COOKIE['m']){
		$m=md5(time().rand(1,1000000));
		setcookie('m',$m,time()+(24*3600*180),'/');
	}
	if($Machines_ID=q("SELECT ID FROM system_machines WHERE UniqueKey='$m'", O_VALUE)){
		//OK
		$updateMachine=true;
	}else if($Machines_ID=q("SELECT ID FROM system_machines WHERE UserAgent='".addslashes($_SERVER['HTTP_USER_AGENT'])."' AND IPAddress='".$_SERVER['REMOTE_ADDR']."' AND MonitorResolution='".$environment."'", O_VALUE)){
		//OK
		$updateMachine=true;
