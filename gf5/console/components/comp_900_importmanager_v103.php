...
2012-05-25:
	...
	* there is a concept that even if I am creating a new table, there can be a "translation" as if it were going into a known table.  
	That allows for the importFieldMap to be universal
...
if(!function_exists('get_row')){
	function get_row($options=array()){
		//2012-06-30 by Sam
		global $SQL, $SQLResult, $fp, $_i_900;
		extract($options);
		if($fp){
			return fgetcsv($fp,100000);
		}else{
		...
		case 'manufacturer':
			...
			if(is_numeric($v)){
				...
				if($ID=q("SELECT ID FROM finan_manufacturers WHERE Name='".addslashes($v)."'", O_VALUE)){
					...
					return q("INSERT INTO finan_manufacturers SET Name='".addslashes($v)."', CreateDate=NOW(), Creator='".($_SESSION['admin']['userName'] ? 
					$_SESSION['admin']['userName'] : ($_SESSION['systemUserName'] ? $_SESSION['systemUserName'] : ($GLOBALS['PHP_AUTH_USER'] ? $GLOBALS['PHP_AUTH_USER'] : 'import')))."'", O_INSERTID);
				}
			}
		break;
		case 'purchaseprice':
...
$UserName=($_SESSION['admin']['userName'] ? $_SESSION['admin']['userName'] : ($PHP_AUTH_USER ? $PHP_AUTH_USER : 'system'));
...
if($submode=='documentation'){
	...
	$_POST_BUFFER=$_POST;
	if($SQL){
		$_SESSION['special']['imports'][$key]['file']='data source';
		$_SESSION['special']['imports'][$key]['filesize']='not given';
	}else{
	...
	if($Data=='(Custom import)'){
		...
		if($importProfile && !strlen($importProfileDescription))
		error_alert('You are choosing to create or update an import profile; you must enter a quick description of this profile');
...
; compare  this to this field in the table: 
	...
	<br />
	<label>
	<input name="doNotOverwrite" type="checkbox" id="doNotOverwrite" value="1" <?php echo !isset($doNotOverwrite) || $doNotOverwrite?'checked':''?> onchange="dChge(this);" />
